...
namespace Grantham\NewsletterGraphQl\Model\Resolver;
...
class UnSubscribeEmailToNewsletter implements ResolverInterface
{
    ...
    /**
     * UnSubscribeEmailToNewsletter constructor.
     *
    ...
    public function resolve(
        ...
        $subscriberCode = $this->subscriberFactory->create()
        ->loadByEmail($email)
        ->getSubscriberConfirmCode();
        ...
        if ($subscriberCode === null) {
             throw new GraphQlInputException(
                 __('Newsletter unsubscribe can not be done.')
             );
        }
        try {
            $subscriber = $this->isCustomerSubscription($email, $currentUserId)
                ...
                : $this->subscriptionManager->unsubscribe($email, $storeId, $subscriberCode);
